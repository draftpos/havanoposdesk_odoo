...
class StockTransfer(models.Model):
    ...
    date = fields.Datetime(string='Date', default=fields.Datetime.now, required=True, readonly=True, states={'draft': [('readonly', False)]})
    from_store_id = fields.Many2one('havanoposdesk.store', string='From Store', required=True, readonly=True, states={'draft': [('readonly', False)]}, domain="[('tenant_id', '=', tenant_id)]")
    to_store_id = fields.Many2one('havanoposdesk.store', string='To Store', required=True, readonly=True, states={'draft': [('readonly', False)]}, domain="[('tenant_id', '=', tenant_id)]")
    state = fields.Selection([
        ('draft', 'Draft'),
        ('done', 'Transferred')
    ], string='Status', default='draft', readonly=True, copy=False)
    
    line_ids = fields.One2many('havanoposdesk.stock.transfer.line', 'transfer_id', string='Transfer Lines', readonly=True, states={'draft': [('readonly', False)]})
            ...
            for line in record.line_ids:
                ...
                if not allow_negative:
                    ...
                    if current_qty < line.qty:
                        raise ValidationError(f"Insufficient stock for '{line.product_id.name}' in Store '{record.from_store_id.name}'. Available: {current_qty}")
...
class StockTransferLine(models.Model):
    ...
    uom_id = fields.Many2one('havanoposdesk.uom', related='product_id.uom_id', string='Unit of Measure', readonly=True)
