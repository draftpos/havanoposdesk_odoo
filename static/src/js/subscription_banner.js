 ...
 * 
 * A dismissible banner that appears at the top of the web client when
 * the tenant's subscription is expiring soon (≤ 3 days) or has expired.
 * 
 * Dismissal is stored in sessionStorage so the banner reappears on next login.
 */
export class SubscriptionBanner extends Component {
    ...
    async fetchSubscriptionInfo() {
        try {
            ...
            console.warn("SubscriptionBanner: Could not fetch info", e);
        }
    }
...
// Register as a systray item so it injects into the web client
